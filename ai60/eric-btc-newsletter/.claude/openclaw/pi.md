...
Image injection is prompt-local: OpenClaw loads image refs from the current prompt and passes them via `images` for that turn only. It does not re-scan older history turns to re-inject image payloads.
...
OpenClaw also has a local TUI mode that uses pi-tui components directly for interactive terminal experience similar to pi's native mode.
...
| Session storage | `~/.pi/agent/sessions/` | `~/.openclaw/agents/<agentId>/sessions/`                                                       |
